"""Measured source-preserving reduction and rigid alignment of the sabertooth surface.
...
p = argparse.ArgumentParser(); p.add_argument('--revision', default='01'); p.add_argument('--flip', action='store_true')
args = p.parse_args(sys.argv[sys.argv.index('--')+1:])
model = ROOT/'output/model-generation/models/sabertooth-tiger'
...
samples = base.sample_surface(P, F, 18000, 17); metres = spec['bodyDraftLengthMetres']/L; Hm = height*metres
trials = []; chosen = None
def metric(points, t):
...
P[:, 2] -= P[:, 2].min(); P[:, 0] -= (P[:, 0].min()+P[:, 0].max())/2; P[:, 1] -= (P[:, 1].min()+P[:, 1].max())/2
chosen.data.vertices.foreach_set('co', P.astype(np.float32).ravel()); chosen.data.update()
chosen.name = 'SabertoothSurface'; chosen.data.name = 'SabertoothSurface'
...
report = dict(candidate=1, revision=args.revision, source=selected, sha256=hashlib.sha256((out/'candidate.glb').read_bytes()).hexdigest(),
    ...
    alignment={'pcaYawDegrees': pca, 'spineYawDegrees': float(best[1]), 'symmetryScore': float(best[0]), 'headSlabSpan': [spanLow, spanHigh],
               'headAtHighYBeforeTurn': bool(headAtHighY), 'manualFlip': args.flip, 'totalYawDegrees': yaw, 'metresPerUnit': metres},
