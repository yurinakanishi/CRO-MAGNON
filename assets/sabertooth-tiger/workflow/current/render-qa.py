"""Exact exported crow GLB pose sheets and optionally six 30 fps videos."""
...
spec={'location':(3.6,-5,1.6),'target':(0,0,.7),'lens':50};rig,mesh=sheet.setup(path,420,480,spec);scene=bpy.context.scene;scene.render.engine='BLENDER_EEVEE';scene.render.fps=60;scene.render.fps_base=1
...
records=[]
for name in (args.clips.split(',') if args.clips else ['Idle_Loop','Walk_Loop','Run_Loop','Gather','Craft','Give','Eat','Wave','Attack']):
    ...
    records.append({'clip':name,'durationSeconds':(end-start)/60,'loop':loop,'views':viewrecords,'boundsBlender':{'min':lo.tolist(),'max':hi.tolist()}});print('FANTASY_QA_COMPLETE',name,flush=True)
report={'source':str(path),'sha256':hashlib.sha256(path.read_bytes()).hexdigest(),'fps':60,'fpsSetBeforeImport':True,'renderEngine':'Blender EEVEE','dimensions':[420,480],'clips':records,'geometryLineage':'Exact original TRELLIS-derived skinned surface; no placeholders','videos':args.videos};(out/'render-manifest.json').write_text(json.dumps(report,indent=2)+'\n')
