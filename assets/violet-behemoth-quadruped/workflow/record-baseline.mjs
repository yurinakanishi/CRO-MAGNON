...
assert.equal(await access(`${out}/baseline.json`).then(() => true, () => false), false);
const files = [];
...
for (const name of ['asset.json']) await copyFile(`public/models/violet-behemoth/${name}`, `${out}/retained-original/${name}`);
await copyFile('shared/behemoth-rules.mts', `${out}/retained-original/behemoth-rules.mts`);
await writeFile(`${out}/baseline.json`, JSON.stringify({ capturedAt: new Date().toISOString(), files }, null, 2) + '\n');
console.log(`Recorded ${files.length} existing GLBs; the adoption must preserve every one byte-for-byte.`);
