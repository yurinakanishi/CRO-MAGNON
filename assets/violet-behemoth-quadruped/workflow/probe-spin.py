...
clip=sys.argv[2] if len(sys.argv)>2 else 'TailSpin'
animation=next(a for a in r.animations() if a['name']==clip)
tr=r.tracks(animation); rows=[]
for t in np.linspace(0,r.clip_bounds(animation)[1],31):
