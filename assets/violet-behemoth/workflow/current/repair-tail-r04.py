...
source=M/'work/low-poly/revision-01/candidate.glb';out=M/'work/low-poly/revision-04';out.mkdir(parents=True,exist_ok=True);assert not (out/'candidate.glb').exists()
obj=base.import_mesh(source);P=base.positions(obj.data);Q=P.copy();rows=[]
amount=np.clip((-P[:,0]-.1)/.8,0,1)*np.clip((P[:,1]-.55)/.65,0,1);P[:,1]-=amount*.95*np.maximum(0,P[:,1]-.55);Q=P.copy()
