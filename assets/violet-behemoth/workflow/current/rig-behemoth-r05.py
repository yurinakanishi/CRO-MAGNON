...
Revision 05/06 (2026-09-12): three telegraph clips so every strike is announced.
Roar (1.2 s) rears the front up with the jaw wide before a charge, Tremble
(0.9 s) shudders the body while the tail lifts and lashes before the tail spin,
Gape (0.6 s) pulls the head back with the mouth open before a bite. All three
start and end in the rest pose so the strike clip that follows joins without a
jump. Surface, weights and the nine earlier clips are unchanged from 04."""
...
durations={'Idle_Loop':3,'Walk_Loop':1.2,'Run_Loop':.8,'Alert':.7,'Roar':1.2,'Charge':2.4,'Gape':.6,'Attack':1,'Tremble':.9,'TailSpin':1.5,'Hit':.3,'Death':1.6}
smooth=lambda t:(lambda v:v*v*(3-2*v))(min(1,max(0,t)))
...
for name,duration in durations.items():
    ...
    for frame in range(n+1):
        ...
        if name=='Idle_Loop':
            ...
            # Rear the front up on braced forearms, jaw wide and quivering, tail lifted and lashing.
            rise=smooth(u/.28)*(1-smooth((u-.78)/.22))
            pose['Chest']={'pitch':-14*rise,'roll':2*math.sin(sec*2*math.pi*3)*rise}
            pose['Head']={'pitch':-24*rise,'yaw':4*math.sin(sec*2*math.pi*2.5)*rise}
            pose['Jaw']={'pitch':(28+6*math.sin(sec*2*math.pi*6))*rise}
            for side,s in [('L',1),('R',-1)]:
                pose['UpperArm.'+side]={'pitch':-12*rise,'roll':s*6*rise}
                pose['LowerArm.'+side]={'pitch':8*rise}
            for i in range(7):pose[f'Tail{i+1}']={'pitch':-3.2*rise*(1+i*.15),'yaw':(6+1.5*i)*math.sin(sec*2*math.pi*2.2-i*.55)*rise}
        elif name=='Tremble':
            # Whole-body shudder: the forearms crouch, the tail lifts and lashes side to side.
            # 4 Hz keeps the shudder well inside the 30 fps keys (7 Hz dipped 16 mm between frames).
            k=smooth(u/.25)*(1-smooth((u-.72)/.28));shake=math.sin(sec*2*math.pi*4)
            pose['Hips']={'yaw':3*shake*k}
            pose['Chest']={'roll':3*shake*k,'pitch':3*k,'yaw':2*math.sin(sec*2*math.pi*4+1.3)*k}
            pose['Head']={'yaw':7*math.sin(sec*2*math.pi*4+.7)*k,'pitch':4*k}
            pose['Jaw']={'pitch':10*k}
            for side,s in [('L',1),('R',-1)]:
                pose['UpperArm.'+side]={'roll':s*9*k,'pitch':6*k}
                pose['LowerArm.'+side]={'pitch':-10*k}
            for i in range(7):pose[f'Tail{i+1}']={'pitch':-2.6*k*(1+i*.2),'yaw':(5+2*i)*math.sin(sec*2*math.pi*3.3-i*.5)*k}
        elif name=='Gape':
            # Head drawn back and up, mouth open: the bite is coming.
            # Revision 08: bigger than 07 (the side view barely showed it): the front
            # rises on braced forearms and the head swings back with the mouth wide.
            k=smooth(u/.35)*(1-smooth((u-.7)/.3))
            pose['Chest']={'pitch':-11*k,'yaw':3*math.sin(sec*2*math.pi*3)*k}
            pose['Head']={'pitch':-32*k}
            pose['Jaw']={'pitch':34*k}
            for side,s in [('L',1),('R',-1)]:
                pose['UpperArm.'+side]={'pitch':-10*k,'roll':s*4*k}
                pose['LowerArm.'+side]={'pitch':6*k}
            for i in range(7):pose[f'Tail{i+1}']={'yaw':3*math.sin(sec*2*math.pi*2-i*.4)*k}
    ...
    for frame in range(n+1):
        ...
        lift=.012 if name=='Tremble' else .002  # the shudder dips between 30 fps keys; keep it above the 6 mm check
        bone=rig.pose.bones['Hips'];bone.location+=inverse@Vector((0,0,-minimum+lift));bone.keyframe_insert(data_path='location',frame=frame)
