...
source=M/'work/low-poly/revision-04/candidate.glb'
...
def p(x,y,z):return [x*W,y*L,z*H]
...
for i in range(5):
    a=.08+i*.08;b=a+.08
    plan.append((f'Tail{i+1}',p(0,a,max(.02,.26-i*.047)),p(0,b,max(.015,.26-(i+1)*.047)),'Hips' if i==0 else f'Tail{i}',False))
...
t=np.clip((P[:,1]/L-.08)/.08,0,4);target=np.zeros_like(weights)
for i in range(5):target[:,names.index(f'Tail{i+1}')]=np.maximum(0,1-abs(t-i))
target/=np.maximum(target.sum(1)[:,None],1e-9);blend(target,ramp(.045*L,.13*L,P[:,1]))
...
weights=sw.laplacian_smooth(weights,starts,neighbours,90,.45)
...
for name,duration in durations.items():
    ...
    for frame in range(n+1):
        ...
        if name=='Idle_Loop':
            ...
            for i in range(5):pose[f'Tail{i+1}']={'yaw':1.2*math.sin(u*2*math.pi-i*.4)}
            ...
            for i in range(5):pose[f'Tail{i+1}']={'yaw':(8 if run else 3.5)*math.sin(t-i*.55)}
        elif name=='Alert':
            pose['Chest']={'pitch':-3*amp};pose['Head']={'pitch':-3*amp}
            for i in range(5):pose[f'Tail{i+1}']={'yaw':13*math.sin(u*4*math.pi-i*.5)*amp}
        elif name=='Attack':
            ...
            spin=np.clip((sec-.45)/.85,0,1)*360
            # Internal body yaw, not world translation; full 360 degrees is sampled.
            pose['Hips']={'yaw':float(spin)}
            for i in range(5):pose[f'Tail{i+1}']={'yaw':-5*amp,'pitch':-2*amp}
            ...
            pose['Hips']={'roll':float(62*k)};pose['Head']={'pitch':float(12*k)}
            for side,s in [('L',1),('R',-1)]:pose['UpperArm.'+side]={'roll':float(s*16*k),'pitch':float(18*k)}
