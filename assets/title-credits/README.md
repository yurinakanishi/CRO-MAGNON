...
## 2026-09-23 制作・友情出演の上下余白を画面全体で均等にする

ユーザーの追加指定で、制作を左・友情出演を右に保ち、各グループを画面全体の縦中央へ修正。
前版はタイトルとメニューのある上段だけを基準に中央配置していたため、画面の上へ寄っていた。
左右のグリッド領域を監修欄まで通し、上の余白30pxと下のフッター30pxを揃えた。
監修4名は中央列の下段に横並びとし、左右の大きい制作欄と重ならないようにした。
アイコン・QR・文字のサイズと元画像は前版を維持。

実Chrome8寸法で確認。1280×800では制作の上下各223.2px、友情出演の上下各297.7px。
1440×900・1920×1080・1000×700・1536×776でも各グループの上下余白差0.1px未満。
767×722・844×390は全スクロール領域を基準に中央配置し、390×844はタイトルの下の同じ行に配置。
いずれも横はみ出し・重なりなし。縦長・低い横画面はスクロールして監修の最後まで到達した。
開始・人物選択から戻る、あそびかたと戻る、上下キー、再読込、12画像の読込を確認し、新規警告・エラー0。
計測 `centered-layout-2026-09-22.json`、通常3000番の14配信SHA一致は `centered-layout-delivery-2026-09-22.json`。
型／strictを含むビルド・373JS構文・依存境界・既存タイトル6テスト・同梱起動4項目が通過。
物理コントローラーと実展示画面の目視は未検証。

`exhibition-20260922-title-center-r01` をPC0・PC1・PC2へ配置・選択し、各410ファイル・78GLBを照合。
ビルド `94b629cb06a335699c5ae9e4572f99241bc4fb0d4f3562214c584836cbf4f84b`。
ZIP SHA `3b28f5221bbaff1fbc7bc30d644949c7b0bd07497d49a1789206aa2c33205944`。
PC1ホスト・PC2クライアントを標準タスクで再起動、PC0は同版で待機、PC3は今回未配布を継続。
配布記録 `output/exhibition-fleet/20260922-235821-deploy-9537afebf9de4bf491d82029b41fde61.json`。
切替後も3台の全ファイルと同一ビルドを再照合した。
再照合 `output/exhibition-fleet/20260923-000110-verify-410bdd32cfca4d5e8ebb96ddd4652a5f.json`。
実PC1/PC2の同梱Nodeから独立部屋 `FLEET-QA-000138` に接続し、双方の移動・攻撃同期と
各12配信ファイルのSHA一致を確認（PC1で28、PC2で19スナップショット）。
記録 `output/exhibition-title-center/FLEET-QA-000138-PC1.json` と `FLEET-QA-000138-PC2.json`。
UAC表示・公開サイトへのデプロイなし。
