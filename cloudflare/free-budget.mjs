...
export const FREE_LIMITS = Object.freeze({ activeMs: 12 * 60 * 60 * 1000,
  messages: 400000, connections: 2000, writes: 20000 });
...
export function reserve(budget, kind, amount, now = Date.now(), limits = FREE_LIMITS) {
  if (budget.day !== utcDay(now)) Object.assign(budget, freshBudget(now));
  if (!(kind in limits) || !Number.isFinite(amount) || amount < 0) throw new Error('Invalid reservation');
