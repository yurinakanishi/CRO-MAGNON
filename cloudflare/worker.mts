...
const ROOM = 'EMBER';
const unavailable =
  '無料の利用上限に達したか、保存サービスが利用できません。ゲームを停止しました。時間をおいて再接続してください。';
const json = (body, status = 200) =>
  Response.json(body, { status, headers: { 'Cache-Control': 'no-store' } });

export class FreeGameRoom extends DurableObject<Env> {
  declare sockets: Set<GameSocket>;
  declare chain: Promise<void>;
  declare timer: number | null;
  declare ticking: boolean;
  declare blocked: string | null;
  declare messageCredit: number;
  declare activeUntil: number;
  declare lastSave: number;
  declare game: ReturnType<typeof createGameCore>;
  declare budget: ReturnType<typeof freshBudget>;
  declare lastCritical: string;
  ...
  constructor(ctx: DurableObjectState, env: Env) {
    ...
    this.ticking = false;
    this.blocked = null;
    this.messageCredit = 0;
    this.activeUntil = 0;
    this.lastSave = 0;
    this.game = createGameCore({
      resumeGraceMs: 7 * 86400000,
      keepEmptyRooms: true,
    ...
    ctx.blockConcurrencyWhile(async () => {
      ...
      const rows = ctx.storage.sql
        .exec<{ data: string }>('SELECT data FROM checkpoint WHERE id = 1')
        .toArray();
  ...
  flush() {
    for (const socket of this.sockets) socket.flush();
  }
  ...
  stop(code = 'FREE_LIMIT') {
    ...
    for (const socket of this.sockets) {
      ...
      try {
        socket.ws.send(JSON.stringify({ type: 'error', code, text: unavailable }));
        socket.ws.close(4008, 'Service paused');
      } catch {}
    }
  }
  ...
  criticalState() {
    ...
    return JSON.stringify({
      camp: room.camp,
      resources: room.resources.map((r) => r.amount),
    ...
    return this.enqueue(async () => {
      ...
      if (url.pathname === '/api/status')
        return json({
          ok: true,
          room: ROOM,
          players: this.game.rooms.get(ROOM)?.players.size || 0,
          maxPlayers: 5,
        });
      if (request.headers.get('Upgrade')?.toLowerCase() !== 'websocket')
        return json({ ok: false }, 426);
      if (!reserve(this.budget, 'connections', 1)) {
        this.stop();
        return json({ ok: false, text: unavailable }, 503);
      }
      this.reserveMinute();
      await this.persist();
      ...
      ws.addEventListener('message', (event) => {
        void this.enqueue(async () => {
          ...
          if (this.messageCredit <= 0) {
            if (!reserve(this.budget, 'messages', 1000)) {
              this.stop();
              return;
            }
            await this.persist();
            this.messageCredit = 1000;
          }
          this.messageCredit--;
          if (
            typeof event.data !== 'string' ||
            new TextEncoder().encode(event.data).length > 2048
          ) {
      ...
      const disconnect = () => {
        void this.enqueue(async () => {
          ...
          if (!this.blocked) {
            await this.persist();
            this.lastCritical = this.criticalState();
            this.flush();
          }
  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === '/ws' || url.pathname === '/api/status') {
      if (env.SERVICE_ENABLED !== 'true')
        return json({ ok: false, text: '現在メンテナンス中です。' }, 503);
      ...
      if (url.pathname === '/ws') {
        ...
        if (room !== ROOM)
          return json({ ok: false, text: '無料公開版の部屋名は EMBER です。' }, 400);
        if (url.search.length > 1000) return json({ ok: false }, 400);
      }
      try {
        return await env.GAME.getByName(ROOM, { locationHint: 'apac' }).fetch(request);
      } catch {
        return json({ ok: false, code: 'FREE_LIMIT', text: unavailable, retryAt: nextDay() }, 503);
      }
    }
