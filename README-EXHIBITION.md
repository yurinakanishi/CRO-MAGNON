...
展示の同時参加は2人です。左上には参加人数に応じて `1/2`・`2/2` を表示し、
日数の表示は `1日目` に固定します。世界の内部時計はそのまま進みます。
左下の「焚き火の会話」とチャット入力は展示モードでは表示しません。
通常モードの参加上限・日数・チャットは従来どおりです。
...
8. 両方でブラウザーが開きます。「はじめる」→キャラクターを選択→
   写真つきの「どこから はじめますか？」で行き先（拠点・マンモス・剣牙の大虎・
   紫尾の巨獣・大聖城・壁画の洞窟）を選ぶと、その場所の前から始まります。
   難易度の質問と確認画面はありません。名前はプレイヤー1 / プレイヤー2、部屋はEXHIBITIONに固定され、入力欄は表示されません。
   両方の画面で **LAN: Connected** を確認します。
9. プレイヤー1 / プレイヤー2が互いに見えることを確認し、交互に歩く・走る・
   向きを変える・攻撃する・採集する操作を行います。

展示ルール（`shared/room-rules.mts` の `EXHIBITION_RULES`、1〜2分の体験向け）:
常に走り・徒歩の速さ1.4倍、与ダメージ4倍、被ダメージ0.2倍で体力は1未満に
ならず倒れない、攻撃と採集で体力を消費しない、大聖城の階級の結界なし、
生き物は10秒で復活。行き先の写真は `node scripts/capture-spawn-art.mjs` で
実際のゲーム画面から `public/spawn/` に撮り直せます。
...
展示LANのメニューは「持ち物」「キャラクターを変える」「ワープする」の3タブです。
ワープは開始時と同じ6枚の写真から選び、持ち物を保ってその場所へ移動します。
キャラクター変更は現在地・持ち物・体力・進行を保ちます。
「タイトルへ戻る」は残し、「探索に戻る」と右上の閉じるボタンは表示しません。
メニューはPS4の×／Switch ProのB（右側4ボタンの下）、メニューボタン、Escで閉じます。
持ち物の操作中やキャラクターの確認中は、下のボタンでひとつ前へ戻ります。
通常オンライン版のメニュー構成は従来どおりです。
