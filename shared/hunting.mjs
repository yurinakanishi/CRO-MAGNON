...
export const HUNTING = Object.freeze({
  ...COMBAT, maxHealth: 100, deathDurationMs: 1200, meatPerAnimal: 4,
  harvestRange: 2.2, respawnMs: 90000, cookRange: 3.4, cookDurationMs: 3000,
  cookedMeatEnergy: 45, inventoryLimit: 99,
});
...
export function nearestCookingFire(state,player) {
  ...
  return fires.reduce((nearest,fire)=>!nearest||huntingDistance(player,fire)<huntingDistance(player,nearest)?fire:nearest,null);
}
...
export function nearestHuntTarget(animals, player, phase = 'alive') {
  return animals.filter(animal => animal.phase === phase)
...
export function initialHuntState(now = Date.now()) {
  return { phase: 'alive', health: HUNTING.maxHealth, maxHealth: HUNTING.maxHealth, meatRemaining: 0, phaseStartedAt: now, hitUntil: 0 };
}
const response = (text, tone = 'error', changed = false) => ({ text, tone, changed });
const obstacles = actors => actors.map(actor => ({ id: actor.id, type: 'circle', x: actor.x, z: actor.z, radius: actor.radius }));
...
export function handleHuntingAction(room, player, message, now = Date.now()) {
  ...
  if (action === 'attack') {
    ...
    if (!result.accepted) return response(profile.key === 'spear' ? '槍を構え直しています。' : '次の攻撃を準備しています。', 'info');
    return response(result.interruptedCooking ? `調理を中止。${profile.startText}生肉は手元に残っています。` : profile.startText, 'info', true);
  }
  ...
  if (action === 'harvest') {
    ...
    if (huntingDistance(player, animal) > HUNTING.harvestRange) return response('肉に近づいてから拾おう。');
    if (!room.collision.segmentFree(player, animal, .12)) return response('岩や小屋に遮られています。回り込もう。');
    if (player.inventory.rawMeat >= HUNTING.inventoryLimit) return response('生肉の持ち物がいっぱいです。');
    ...
    if (!animal.meatRemaining) { animal.phase = 'respawning'; animal.phaseStartedAt = now; }
  ...
  if (!player.inventory.cookedMeat) return response('焼いた肉を持っていません。生肉は焚き火で焼こう。');
...
export function updateHunting(room, now = Date.now(), notify = () => {}) {
  let changed = false;
  const reportHit = (player, strike) => {
    changed = true;
    const label = strike.kind === 'animal' ? 'マンモス' : strike.target.name || '敵';
    if (strike.killed) notify(player, strike.kind === 'animal' ? 'マンモスを倒した！ 肉になったら近づいて拾おう。' : `${label}を倒した！`, 'success');
    else notify(player, `${attackProfile(player).noun}が命中！ ${label} ${strike.target.health} / ${strike.target.maxHealth}`, 'success');
  };
  for (const player of room.players.values()) {
    const strike = resolveAttack(room, player, now);
    if (strike?.hit) reportHit(player, strike);
    if (strike?.launched) changed = true;
    if (!player.cookingEndsAt) continue;
    if (huntingDistance(player, nearestCookingFire(room,player)) > HUNTING.cookRange) {
      player.cookingEndsAt = 0; changed = true;
  ...
  for (const animal of room.animals) {
    if (animal.phase === 'dying' && now - animal.phaseStartedAt >= HUNTING.deathDurationMs) {
      animal.phase = 'meat'; animal.phaseStartedAt = now; animal.meatRemaining = HUNTING.meatPerAnimal;
      animal.clip = null; changed = true;
    } else if (animal.phase === 'respawning' && now - animal.phaseStartedAt >= HUNTING.respawnMs) {
      const dynamic = obstacles([...room.players.values(), ...room.animals.filter(other => other !== animal && animalIsSolid(other)), ...(room.enemies||[]).filter(enemyIsSolid)]);
      const spawn = room.collision.nearestFree(animal.home, animal.radius, dynamic, animal.roamRadius);
      if (!spawn) continue;
      Object.assign(animal, spawn, initialHuntState(now)); stopActor(animal);
      animal.nextRoam = animal.age + 1; animal.clip = 'Idle_Loop'; changed = true;
    }
  }
  return changed;
}
