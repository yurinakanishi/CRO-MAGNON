...
export function sampleHeightField(field, u, v) {
  ...
  u = clamp(u, 0, 1) * (n - 1); v = clamp(v, 0, 1) * (n - 1);
  const ix = Math.min(n - 2, Math.floor(u)), iz = Math.min(n - 2, Math.floor(v)), fx = u - ix, fz = v - iz;
  return (heights[iz*n+ix]*(1-fx)+heights[iz*n+ix+1]*fx)*(1-fz)
...
export function installBiomeTerrain(fields) {
  for (const biome of BIOMES) {
    ...
    if (!field || !Number.isInteger(field.resolution) || field.resolution < 2 || field.heights?.length !== field.resolution ** 2 || !field.heights.every(Number.isFinite)) {
      throw new Error(`Missing verified terrain samples: ${biome.id}`);
    }
  }
  const sampler = (x, z) => {
    const cell = chunkAt(x, z), chunk = chunkDescription(cell.x, cell.z);
    const dx=x-chunk.x,dz=z-chunk.z,c=Math.cos(chunk.yaw),s=Math.sin(chunk.yaw);
    return sampleHeightField(fields[chunk.biome], (c*dx-s*dz)/WORLD.chunkSize+.5, (s*dx+c*dz)/WORLD.chunkSize+.5);
  };
  sourceSurface = sampler;
  return () => { if(sourceSurface === sampler) sourceSurface = null; };
}
