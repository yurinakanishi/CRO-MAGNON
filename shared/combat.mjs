...
export const COMBAT = Object.freeze({
  attackDamage: 25, attackCooldownMs: 850, attackDurationMs: 700,
  attackImpactMs: 333, spearReach: 1.65, halfAngleRadians: 55 * Math.PI / 180,
});
export const combatDistance = (a, b) => Math.hypot(a.x - b.x, a.z - b.z);
export const withinSpearReach = (player, target) => combatDistance(player, target) <= (player.radius ?? WORLD.playerRadius) + target.radius + COMBAT.spearReach;
export const withinAttackReach = (player, target) => {
  const profile=attackProfile(player);
  return combatDistance(player, target) <= target.radius + profile.reach + (profile.projectileRadius ?? player.radius ?? WORLD.playerRadius);
};
export const enemyIsSolid = enemy => enemy.hostile === true && (enemy.phase === 'alive' || enemy.phase === 'dead');
export function stopActor(actor) {
  actor.dx = 0; actor.dz = 0; actor.target = null; actor.path = [];
  actor.navigationGoal = null; actor.navigationEnd = null;
  actor.moving = false; actor.running = false; actor.speed = 0;
}
const alive = actor => Number.isFinite(actor.health) && actor.health > 0 && actor.alive !== false && (actor.phase === undefined || actor.phase === 'alive');
const actors = collection => collection instanceof Map ? [...collection.values()] : collection || [];
...
export function startAttack(room, player, message = {}, now = Date.now()) {
  ...
  if (aimed && withinAttackReach(player, aimed) && clearLine(room, player, aimed)) player.facing = Math.atan2(aimed.x - player.x, aimed.z - player.z);
  ...
  player.pendingStrike = { facing: player.facing, impactAt: now + profile.impactMs, kind: profile.key };
...
export function resolveAttack(room, player, now = Date.now()) {
  ...
  const hit = damageableTargets(room)
    .filter(({ target }) => withinAttackReach(player, target) && inAttackArc(player, target, strike.facing) && clearLine(room, player, target))
...
function applyHit({ target, kind }, profile, now) {
  ...
  if (kind === 'enemy') {
    target.hitSequence = (target.hitSequence || 0) + 1; target.hitAt = now;
    target.pendingAttack = null; target.attackLockUntil = 0;
  }
  if (killed) {
    target.phase = kind === 'animal' ? 'dying' : 'dead'; target.phaseStartedAt = now;
...
export function circleEntry(a, b, centre, radius) {
  const dx=b.x-a.x, dz=b.z-a.z, ox=a.x-centre.x, oz=a.z-centre.z;
  const c=ox*ox+oz*oz-radius*radius;
  if (c<=0) return 0;
  const length2=dx*dx+dz*dz;
  if (length2<1e-12) return Infinity;
  const dot=ox*dx+oz*dz, discriminant=dot*dot-length2*c;
...
function boxEntry(a, b, box, radius) {
  const x=box.c*(a.x-box.x)-box.s*(a.z-box.z), z=box.s*(a.x-box.x)+box.c*(a.z-box.z);
  const dx=box.c*(b.x-a.x)-box.s*(b.z-a.z), dz=box.s*(b.x-a.x)+box.c*(b.z-a.z);
  let lo=0,hi=1;
  for (const [p,d,half] of [[x,dx,box.hx+radius],[z,dz,box.hz+radius]]) {
    if (Math.abs(d)<1e-12) { if(Math.abs(p)>half)return Infinity; }
    else { const u=(-half-p)/d,v=(half-p)/d;lo=Math.max(lo,Math.min(u,v));hi=Math.min(hi,Math.max(u,v)); }
  }
...
export function projectileWallEntry(collision, a, b, radius) {
  ...
  for (const axis of ['x','z']) {
    const min=WORLD[axis==='x'?'minX':'minZ']+WORLD.edgeMargin,max=WORLD[axis==='x'?'maxX':'maxZ']-WORLD.edgeMargin;
    if(b[axis]<min || b[axis]>max) {
      const edge=b[axis]<min?min:max,delta=b[axis]-a[axis];
...
export function updateProjectiles(room, now=Date.now()) {
  ...
  for (const projectile of room.projectiles || []) {
    ...
    const travel=Math.min(profile.reach-projectile.travelled,Math.max(0,now-projectile.updatedAt)*profile.projectileSpeed/1000);
