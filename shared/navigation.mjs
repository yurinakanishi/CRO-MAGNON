...
export function planNavigation(actor,goal,collision,dynamic,now=Date.now()) {
  actor.navigationGoal={x:goal.x,z:goal.z};actor.navigationEnd=null;
  actor.target=null;actor.path=[];actor.nextNavigationAt=0;
...
export function updateNavigation(actor,collision,dynamic,now=Date.now()) {
  ...
  if(!actor.target&&actor.navigationEnd&&distance(actor,actor.navigationEnd)<.015) {
    actor.navigationGoal=null;actor.navigationEnd=null;return false;
  }
  if(now<actor.nextNavigationAt)return !!actor.target;
  if(actor.target&&collision.segmentFree(actor,actor.target,actor.radius,dynamic))return true;
