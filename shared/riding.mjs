...
export const RIDING = Object.freeze({ version: 1, reach: 1.35, walkSpeed: .7, runSpeed: 2.25 });
const circle = actor => ({ id: actor.id, type: 'circle', x: actor.x, z: actor.z, radius: actor.radius });
export const mountedAnimal = (room, player) => room.animals.find(a => a.id === player.mountId && a.riderId === player.id && a.phase === 'alive');
export const ridingDistance = (player, animal) => combatDistance(player, animal) - animal.radius - player.radius;
export function ridingObstacles(room, animal, rider) {
  return [...room.players.values()].filter(p => p !== rider && !p.mountId)
    .concat(room.animals.filter(a => a !== animal && ['alive', 'dying'].includes(a.phase)), (room.enemies || []).filter(enemyIsSolid)).map(circle);
}
...
export function dismountPoint(room, player, animal) {
  ...
  for (let i = 0; i < 16; i++) {
    ...
    if (room.collision.free(point, player.radius, dynamic) && room.collision.segmentFree(animal, point, player.radius)) return point;
  }
  return null;
}
export function handleRidingAction(room, player, message, now) {
  ...
  if (player.attackSequence && now - player.attackAt < attackProfile(player).durationMs) return fail('攻撃が終わってから乗ろう。');
  const animal = typeof message.targetId === 'string'
    ? room.animals.find(a => a.id === message.targetId)
    : room.animals.filter(a => a.phase === 'alive' && !a.riderId).sort((a, b) => ridingDistance(player, a) - ridingDistance(player, b))[0];
  if (!animal || animal.phase !== 'alive' || animal.health <= 0) return fail('生きているマンモスの横で R を押すと乗れます。');
  if (animal.riderId) return fail('このマンモスには仲間が乗っています。1頭につき1人乗れます。');
  if (ridingDistance(player, animal) > RIDING.reach || !room.collision.segmentFree(player, animal, .12)) return fail('マンモスの横まで近づいて R を押そう。');
  if (now < animal.hitUntil) return fail('マンモスが落ち着いてから乗ろう。');
  stopActor(player); stopActor(animal);
  player.pendingStrike = null; player.cookingEndsAt = 0;
  player.mountId = animal.id; animal.riderId = player.id;
  animal.lastInput = now; animal.runningRequested = false; animal.clip = 'Idle_Loop';
  copyRiderPosition(player, animal);
  return { changed: true, tone: 'success', text: 'マンモスに乗りました！ WASDで移動・Shiftで走る・Rで降りる。' };
}
export function updateRiddenAnimal(room, animal, dt, now) {
  ...
  if (!rider || rider.mountId !== animal.id) { release(animal); return false; }
  ...
  movePlayer(animal, dt, now, (p, dx, dz) => room.collision.move(p, dx, dz, animal.radius, dynamic), speed);
