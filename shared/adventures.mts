...
function discover(player, region, notify = (_player, _text: string, _tone?: string) => {}) {
...
export function recordAdventureGather(player, resource, amount) {
  ...
  if (!region || resource.type !== region.material || !Number.isInteger(amount) || amount <= 0)
    return;
  const progress = discover(player, region);
  progress.gathered = Math.min(region.amount, progress.gathered + amount);
}
export function updateAdventures(
  room,
  now,
  ...
  for (const player of room.players.values()) {
    ...
    for (const point of region.checkpoints)
      if (
        !progress.visited.includes(point.id) &&
        Math.hypot(point.x - player.x, point.z - player.z) <= 3.4 &&
        room.collision.segmentFree(player, point, 0.12)
      ) {
        progress.visited.push(point.id);
        notify(player, `道標：${point.name}（${progress.visited.length}/3）`, 'success');
      }
  }
  for (const enemy of room.enemies ?? []) {
    if (!enemy.regionId) continue;
    if (enemy.phase === 'alive') {
      enemy.adventureRewarded = false;
      continue;
    }
    if (enemy.phase !== 'dead' || enemy.adventureRewarded) continue;
    enemy.adventureRewarded = true;
    const region = regionById(enemy.regionId);
    if (!region) continue;
    for (const player of room.players.values())
      if (
        !player.downedUntil &&
        !player.mountId &&
        !player.boatId &&
        Math.hypot(player.x - enemy.x, player.z - enemy.z) <= 24
      ) {
        const progress = discover(player, region, notify);
        progress.defeated ??= [];
        if (progress.defeated.includes(enemy.id)) continue;
        progress.defeated.push(enemy.id);
        progress.kills = progress.defeated.length;
        notify(
          player,
          `${region.name}の討伐を記録（${progress.kills}/${region.kills}）。近くの仲間と共有しました。`,
          'success',
        );
      }
  }
...
export function handleAdventureAction(room, player, message, now = Date.now()) {
  ...
  if (player.downedUntil || player.mountId || player.boatId)
    return { ok: false, text: '回復して、船やマンモスから降りてから行おう。' };
  if (message.action === 'rift') {
    const rift = RIFTS.find((r) => r.id === message.targetId);
    if (
      !rift ||
      Math.hypot(player.x - rift.x, player.z - rift.z) > 4 ||
      !room.collision.segmentFree(player, rift, 0.12)
    )
      return { ok: false, text: '裂け目の光へ近づこう。' };
    if (!rift.exit && travelSeals(player) < 3)
      return { ok: false, text: '影の世界へは、地上の地域依頼を3つ達成して旅の証を集めよう。' };
    if (now - (player.lastExpeditionAt ?? -Infinity) < 3000)
      return { ok: false, text: '次の移動まで少し待ってください。' };
    const destination = rift.exit
      ? { x: RIFTS[0].x, z: RIFTS[0].z + 2 }
      : regionById(rift.destination).camp;
    ...
    return {
      ok: true,
      text: rift.exit ? '地上の石の森へ戻りました。' : '裂け目を抜け、影の庭に到着しました。',
    };
  }
  const region = regionAt(player.x, player.z);
  ...
  if (message.action === 'rest') {
    ...
    if (
      !fire ||
      Math.hypot(player.x - fire.x, player.z - fire.z) > 4 ||
      !room.collision.segmentFree(player, region.camp, 0.12)
    )
      return { ok: false, text: 'この地域の焚き火へ近づこう。' };
    if (player.energy >= 100) return { ok: false, text: '元気いっぱいです。' };
    if (now - (player.lastRestAt ?? -Infinity) < 20000)
      return { ok: false, text: '次の休息まで20秒ほど間をあけよう。' };
    if (
      room.enemies.some(
        (e) => e.phase === 'alive' && Math.hypot(e.x - player.x, e.z - player.z) < 12,
      )
    )
      return { ok: false, text: '敵が近くにいます。安全な場所へ戻ろう。' };
    stopActor(player);
    player.lastRestAt = now;
    player.energy = Math.min(100, player.energy + 35);
    return { ok: true, text: '焚き火でひと休み。元気 +35。' };
  }
  if (
    message.targetId !== region.id ||
    Math.hypot(player.x - region.camp.x, player.z - region.camp.z) > 6 ||
    !room.collision.segmentFree(player, region.camp, 0.12)
  )
    return { ok: false, text: '依頼を終えた地域の野営地へ戻ろう。' };
  ...
  return {
    ok: true,
    text: region.realm
      ? '影の庭の探索を達成！「境界を渡る者」の称号と食料を手に入れた。'
      : `${region.name}を達成！ 旅の証 ${travelSeals(player)}/5 と食料を手に入れた。`,
  };
}
