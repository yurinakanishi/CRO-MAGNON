...
セットアップの再実行時は、起動中と確認できた既存の展示版を照合し、標準ランチャーへ
停止を要求する。停止完了後にタスクを再登録し、元々起動していた展示だけを再起動する。
待機機は起動しない。別のNodeやブラウザーを一括終了しない。ゲームの版は変更しない。
ホストの再起動で展示世界はリセットされるため、この保守は体験の区切りで実行する。
旧キットで「Stop the exhibition ... before registering its launcher」と表示された場合は、
この停止・復帰処理を含む最新版のセットアップZIPを全体ごと展開して使う。
