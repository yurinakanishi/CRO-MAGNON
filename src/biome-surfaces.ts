...
const profiles = {
  ...
  'berry-bush': { low: 0.3, rock: 0, plant: 1, hearth: 0 },
  ...
  const models = [template.gltf, ...template.lods].map((model) => {
    ...
    root.traverse((node) => {
      ...
      const uniforms = {
        ...
        regionalProfile: {
          value: new THREE.Vector4(profile.low, profile.rock, profile.plant, profile.hearth),
        },
        regionalMode: { value: modes[surface] },
        regionalHeight: { value: Math.max(0.01, template.asset.heightMetres ?? size.y) },
        regionalRadius: { value: Math.max(0.01, Math.min(size.x, size.z) * 0.5) },
      };
      const coat = (original) => {
        ...
        material.userData = {
          ...original.userData,
          regionalSurface: surface,
          sharedSourceMaterial: original.uuid,
        };
