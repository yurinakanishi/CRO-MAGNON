...
manager.setURLModifier(url => {
  ...
  throw new Error('外部ファイルを参照するモデルです。テクスチャを埋め込んだGLBを使用してください。');
});
const loader = new GLTFLoader(manager);
let model = null, actors = [], selected = null, playing = true, height = 1.8, framingSpan = 1.8, loadId = 0, requestId = 0;
const framingOffset = new THREE.Vector3();
let lastTime = performance.now(), frameCount = 0, sampleStart = lastTime, loadDuration = 0;
...
function clearActors() {
  for (const actor of actors) {
    ...
    actor.root.traverse(object => { if (object.isSkinnedMesh) object.skeleton.dispose(); });
...
function setCamera() {
  ...
  const views = {
    ...
    left: [2 * span, framingSpan * .75, 0], right: [-2 * span, framingSpan * .75, 0],
    tps: [-height * .32, height * .93, -height * 1.5],
    top: [0, span * 2.2, .001],
  };
  orbit.target.set(0, height * (ui.camera.value === 'tps' ? .83 : .5), actors.length === 5 && ui.camera.value !== 'tps' ? 1 : 0);
  if (ui.camera.value === 'tps') orbit.target.set(-height * .32, height * .82, height * .45);
  ...
  if (actors.length && (actors.length > 1 || !model?.animations.length || actors.some(actor => actor.gear)) && ui.camera.value !== 'tps') {
    ...
    for (const x of [bounds.min.x, bounds.max.x]) for (const y of [bounds.min.y, bounds.max.y]) for (const z of [bounds.min.z, bounds.max.z]) {
      ...
      distance = Math.max(distance, depth + Math.abs(corner.dot(right)) * 1.12 / tanH, depth + Math.abs(corner.dot(up)) * 1.12 / tanV);
    }
...
function setClip() {
  ...
  for (const [index, actor] of actors.entries()) {
    ...
    if (selected) {
      ...
      actor.action.setLoop(selected.name.endsWith('_Loop') ? THREE.LoopRepeat : THREE.LoopOnce, Infinity);
...
function populateActors() {
  ...
  for (let i = 0; i < Number(ui.count.value); i++) {
    ...
    if (equipment) {
      ...
      gear.traverse(object => { if (object.isMesh) { object.castShadow = true; object.receiveShadow = true; } });
    }
...
async function openBuffer(buffer, name) {
  const ownLoad = ++loadId, started = performance.now();
  ...
  try {
    ...
    height = Math.max(.2, size.y); framingSpan = Math.max(height, size.x * .8, size.z * .8);
...
ui.equipment.addEventListener('change', async () => {
  ...
  try {
    if (key && !equipmentModels.has(key)) {
      ...
      const asset = await response.json(), gltf = await loadVerifiedGLB(asset);
...
ui.skeleton.addEventListener('change', () => actors.forEach(actor => { actor.helper.visible = ui.skeleton.checked; }));
ui.ground.addEventListener('change', () => { floor.visible = grid.visible = ui.ground.checked; });
...
async function openServedModel(modelPath) {
  ...
  if (url.origin !== location.origin || !url.pathname.startsWith('/models/') || !url.pathname.endsWith('.glb')) {
    ...
    try {
      ...
      if (request === requestId) await openBuffer(bytes, decodeURIComponent(url.pathname.split('/').pop()));
    } catch (error) { if (request === requestId) ui.status.textContent = `読込失敗: ${error.message}`; }
  }
}
