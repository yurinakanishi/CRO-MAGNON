    ...
    root.traverse(node=>{if(!node.isMesh)return;
      const coat=source=>{
        ...
        if(!this.cache.has(id)){
          const material=source.clone();material.name=`${key} / ${surface}`;
          const tint=new THREE.Color({alpine:'#8e929b',ochre:'#b17848',moss:'#627c70',shadow:'#62517f',rift:'#7376a2'}[surface]);
          material.onBeforeCompile=shader=>{
            shader.uniforms.adventureTint={value:tint};
            shader.vertexShader='varying float adventureHeight; varying float adventureTop;\n'+shader.vertexShader;
            shader.vertexShader=shader.vertexShader.replace('#include <begin_vertex>','#include <begin_vertex>\nadventureHeight=position.y;adventureTop=normal.y;');
            shader.fragmentShader='uniform vec3 adventureTint; varying float adventureHeight; varying float adventureTop;\n'+shader.fragmentShader;
          ...
          if(surface==='shadow'||surface==='rift'){material.emissive.set(surface==='shadow'?'#17102c':'#222653');material.emissiveIntensity=.35;}
          this.cache.set(id,{material,key,last:time});
        }
        const entry=this.cache.get(id);entry.last=time;return entry.material;
      };
      node.material=Array.isArray(node.material)?node.material.map(coat):coat(node.material);
    });
  }
  touch(key,time){for(const entry of this.cache.values())if(entry.key===key)entry.last=time;}
  evict(time){for(const[id,entry]of this.cache)if(time-entry.last>12){entry.material.dispose();this.cache.delete(id);}}
  dispose(){for(const entry of this.cache.values())entry.material.dispose();this.cache.clear();}
}
