...
export function createEarthTextures(){
  ...
  coast.minFilter=coast.magFilter=THREE.LinearFilter;coast.generateMipmaps=false;coast.needsUpdate=true;
  const width=512,height=256,data=new Uint8Array(width*height*4),colors=BIOMES.map(b=>new THREE.Color(b.color).toArray());
  for(let z=0;z<height;z++)for(let x=0;x<width;x++){
    const w=geographicWeights(EARTH.minX+(x+.5)/width*EARTH.width,EARTH.minZ+(z+.5)/height*EARTH.height),index=(z*width+x)*4;
    for(let c=0;c<3;c++)data[index+c]=Math.round(w.reduce((sum,weight,i)=>sum+weight*colors[i][c],0)*255);
    data[index+3]=255;
  }
  const biomes=new THREE.DataTexture(data,width,height);biomes.minFilter=biomes.magFilter=THREE.LinearFilter;biomes.generateMipmaps=false;biomes.needsUpdate=true;
  return {coast,biomes,dispose(){coast.dispose();biomes.dispose();}};
}
const earthShader=`
...
export function earthTerrainMaterial(original,biome,textures){
  const material=original.clone(),base=new THREE.Color(biome.color);
  material.roughness=biome.id==='ice'?.28:.94;material.side=THREE.FrontSide;
  material.onBeforeCompile=shader=>{
    shader.uniforms.earthCoast={value:textures.coast};shader.uniforms.earthBiomes={value:textures.biomes};
    shader.vertexShader=earthShader+'varying vec3 vTerrainWorld;\nvarying float vCoastal;\n#ifdef USE_INSTANCING\nattribute float earthCoastal;\n#endif\n'+shader.vertexShader;
    shader.vertexShader=shader.vertexShader.replace('#include <begin_vertex>',`#include <begin_vertex>
    ...
    shader.fragmentShader=earthShader+'uniform sampler2D earthBiomes;\nvarying vec3 vTerrainWorld;\nvarying float vCoastal;\n'+shader.fragmentShader;
    shader.fragmentShader=shader.fragmentShader.replace('#include <color_fragment>',`#include <color_fragment>
...
export class EarthOcean {
  constructor(world,textures,capacity=100){
    this.world=world;this.parts=[];this.matrix=new THREE.Matrix4();this.time={value:0};
    const root=world.worldAssets.get('river-water').gltf.scene;root.updateMatrixWorld(true);
    ...
    root.traverse(node=>{
      ...
      const geometry=node.geometry.clone().applyMatrix4(node.matrixWorld).translate(-centre.x,-box.max.y,-centre.z).scale(34/size.x,.012,34/size.z);
      const material=new THREE.MeshStandardMaterial({color:'#417d90',roughness:.38,metalness:.08,side:THREE.DoubleSide});
      material.onBeforeCompile=shader=>{
        shader.uniforms.earthCoast={value:textures.coast};shader.uniforms.oceanTime=this.time;
        ...
        shader.fragmentShader=earthShader+'uniform float oceanTime;\nvarying vec3 vOceanWorld;\n'+shader.fragmentShader;
        shader.fragmentShader=shader.fragmentShader.replace('#include <color_fragment>',`#include <color_fragment>
      ...
      const mesh=new THREE.InstancedMesh(geometry,material,capacity);mesh.frustumCulled=false;mesh.count=0;mesh.instanceMatrix.setUsage(THREE.DynamicDrawUsage);world.scene.add(mesh);
      this.parts.push({geometry,material,mesh});
    });
  }
  update(chunks,time){
    this.time.value=time;let count=0;
    for(const c of chunks){this.matrix.makeTranslation(c.x,-.52,c.z);for(const p of this.parts)p.mesh.setMatrixAt(count,this.matrix);count++;}
    for(const p of this.parts){p.mesh.count=count;p.mesh.instanceMatrix.needsUpdate=true;p.mesh.boundingSphere=null;}
  }
  dispose(){for(const p of this.parts){this.world.scene.remove(p.mesh);p.mesh.dispose();p.geometry.dispose();p.material.dispose();}}
}
