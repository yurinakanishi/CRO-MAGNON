...
export function installCropFoodUI({
  player,
  state,
  available,
  action,
  ...
  function open() {
    ...
    $('crop-food-fire').onclick = () => {
      const fire = nearestCookingFire(state(), player());
      goTo(fire.x, fire.z + 2, '近くの炉');
    };
  ...
  function update() {
    ...
    enable('crop-food-fire', !blocked && !atFire);
  }
  return { open, update };
}
