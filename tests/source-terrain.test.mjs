...
import { marshDrop } from '../dist/shared/behemoth-rules.mjs';
  ...
  try {
    for (const x of [30, 50, 70, 90])
      assert.ok(
        Math.abs(
          terrainHeight(x, 50) -
            Math.max(1 - riverBankDrop(x, 50) - marshDrop(x, 50), mountainHeight(x, 50)),
        ) < 1e-10,
      );
    assert.equal(terrainHeight(45, 45), 0.5);
    const atRiver = terrainHeight(65, 50),
      withoutBank = atRiver + riverBankDrop(65, 50);
...
test('disposing an old world cannot clear a newer world terrain sampler', () => {
  ...
  try {
    previous();
    assert.equal(terrainHeight(30, 30), 2);
  } finally {
    current();
  }
});
...
test('the measured bridge supports feet only over its actual deck', () => {
  ...
  const old = installSourceBridge({ minX: -5, maxX: 5, minZ: -1.3, maxZ: 1.3 }, 65, 43.5, 0.3);
  const current = installSourceBridge({ minX: -5, maxX: 5, minZ: -1.3, maxZ: 1.3 }, 65, 43.5, 0.4);
  try {
    old();
    assert.equal(walkHeight(65, 43.5), 0.4);
