...
test('all six delivered skins seat at the animated spine, restore their bind transforms and resume walking',async()=>{
  ...
  for(const model of CHARACTER_MODELS){
    const gltf=await geometryScene(`public/models/${model.key}/model.glb`),group=new THREE.Group();group.add(gltf.scene);
    const originalBounds=new THREE.Box3().setFromObject(gltf.scene,true),height=originalBounds.max.y-originalBounds.min.y;
    const pose=new RidingPose(gltf.scene),animation=new CharacterAnimation(gltf.scene,gltf.animations,{walkSpeed:1,runSpeed:3});
    for(let i=0;i<12;i++){
      mixer.update(.1);mammoth.scene.updateMatrixWorld(true);pose.update(animation,i*.1,2.25);
      const pelvis=pose.pelvisOffset(new THREE.Vector3()),seat=socket.position(new THREE.Vector3());group.position.copy(seat).sub(pelvis);group.updateMatrixWorld(true);
      assert.ok(pose.hips.getWorldPosition(new THREE.Vector3()).distanceTo(seat)<1e-5,model.key);
      const bounds=new THREE.Box3().setFromObject(gltf.scene,true);assert.ok(Number.isFinite(bounds.min.x)&&bounds.max.y-bounds.min.y<height*1.2);
      assert.ok(pose.bones.get('LowerLegL').bone.getWorldPosition(new THREE.Vector3()).x>.1,model.key+' left knee');
      assert.ok(pose.bones.get('LowerLegR').bone.getWorldPosition(new THREE.Vector3()).x<-.1,model.key+' right knee');
    }
    ...
    animation.update(.1,1);assert.equal(animation.name,'Walk_Loop');assert.equal(animation.current.isRunning(),true);animation.dispose();
  }
});
