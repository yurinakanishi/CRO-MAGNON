...
test('actual exported humanoid clips cannot overwrite the world heading',async()=>{
  for(const {key} of CHARACTER_MODELS) {
    const gltf=await geometryScene(`public/models/${key}/model.glb`),placement=new THREE.Group();placement.add(gltf.scene);
    const animation=new CharacterAnimation(gltf.scene,gltf.animations,{walkSpeed:1,runSpeed:3});
    for(const heading of [0,Math.PI/2,Math.PI,-Math.PI/2])for(const running of [false,true]) {
      placement.rotation.y=heading;placement.position.set(30,0,40);
    ...
    for (const heading of [0,Math.PI/2,Math.PI,-Math.PI/2]) {
      ...
      for(let frame=0;frame<26;frame++) {
        animation.update(1/30,0,false);placement.updateMatrixWorld(true);
        const forward=new THREE.Vector3(0,0,1).transformDirection(gltf.scene.matrixWorld);
        assert.ok(forward.dot(new THREE.Vector3(Math.sin(heading),0,Math.cos(heading)))>.999);
        assert.deepEqual(placement.position.toArray(),[30,0,40]);
      }
