...
test('missing, invalid, remote and dead IDs do not block swings or redirect them behind the player', () => {
  for (const targetId of [undefined, 'missing', null, 42, {}, ['front'], 'remote', 'dead']) {
    ...
    assert.equal(began.accepted, true); assert.equal(strike.target, front); assert.equal(front.health,  100 - COMBAT.attackDamage);
...
test('a visible nearby click target can aim, while unrequested targets outside the forward arc remain unharmed', () => {
  ...
  assert.equal(Math.abs(player.facing), Math.PI); assert.equal(behind.health,  100 - COMBAT.attackDamage);
...
test('impact selects one nearest target in reach and rechecks moving targets and new occlusion after windup', () => {
  ...
  assert.equal(swing(room, player).strike.target, near); assert.equal(near.health,  100 - COMBAT.attackDamage); assert.equal(far.health, 100);
  room.animals = [near]; near.z = 30;
  startAttack(room, player, {}, 2000); near.z = 22;
  assert.equal(resolveAttack(room, player, 2332), null); assert.equal(near.health,  100 - COMBAT.attackDamage);
  assert.equal(resolveAttack(room, player, 2333).target, near); assert.equal(near.health,  100 - 2 * COMBAT.attackDamage);
  startAttack(room, player, {}, 3000); near.z = 30;
  assert.equal(resolveAttack(room, player, 3333).hit, false); assert.equal(near.health,  100 - 2 * COMBAT.attackDamage);
  ...
  assert.equal(resolveAttack(room, player, 4333).hit, false); assert.equal(near.health,  100 - 2 * COMBAT.attackDamage);
...
test('explicit hostile enemies take damage and die without mammoth meat; friendlies cannot be targeted', () => {
  const { room, player } = fixture();
  const enemy = { ...creature('enemy'), hostile: true, name: '敵', health: COMBAT.attackDamage, maxHealth: COMBAT.attackDamage };
