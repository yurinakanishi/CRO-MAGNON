...
test('active replacement, disconnect and durable restart restore possessions at the safe shore',()=>{
  ...
  try{action(s,p,'craftBoat');action(s,p,'boardBoat');const welcome=s.messages.find(m=>m.type==='welcome');
    ...
    const rp=restored.rooms.get('BOAT-QA').players.get(p.id);assert.equal(rp.boatId,null);assert.equal(rp.inventory.wood,12);assert.equal(rp.x,128);assert.equal(rp.z,124);
    assert.equal(restored.rooms.get('BOAT-QA').boats[0].riderId,null);
    const replacement=new Socket();core.connect(replacement,new URLSearchParams({room:'BOAT-QA',resume:'1',session:welcome.session}));assert.equal(p.boatId,null);assert.equal(p.x,128);assert.equal(p.z,124);assert.equal(b.riderId,null);
