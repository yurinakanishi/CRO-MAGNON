...
test('world rollout restores cat/bear inventories, resources and hunting phases using the current full protocol',async t=>{
  ...
  Object.assign(players[0],{x:-115,z:-96,tool:true,energy:63,inventory:{wood:12,stone:9,berry:4,rawMeat:2,cookedMeat:3}});
  ...
  assert.equal(cat.state.worldVersion,2);assert.equal(cat.state.combatVersion,3);assert.equal(cat.state.characterVersion,2);
  assert.equal(cat.state.cookingFires.length,6);assert.deepEqual(cat.state.projectiles,[]);
