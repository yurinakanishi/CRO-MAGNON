...
test('six exact delivered skins sit on the canoe socket with feet inside its cockpit and restore walking', async () => {
  ...
  for (const profile of CHARACTER_MODELS) {
    ...
    for (let i = 0; i < 20; i++) {
      pose.update(animation, i * 0.1, 7, true);
      group.position.copy(seat).sub(pose.pelvisOffset(new THREE.Vector3()));
      group.updateMatrixWorld(true);
      assert.ok(pose.hips.getWorldPosition(new THREE.Vector3()).distanceTo(seat) < 1e-5);
      for (const name of ['FootL', 'FootR']) {
        const point = pose.bones.get(name).bone.getWorldPosition(new THREE.Vector3());
        assert.ok(Math.abs(point.x) < bounds.max.x - 0.2, profile.key);
        assert.ok(point.y > 0.37, profile.key);
        assert.ok(point.z > seat.z && point.z < bounds.max.z - 0.5, profile.key);
      }
    }
