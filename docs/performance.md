...
`scripts/benchmark-terrain.mjs` は実際の区画選択・海岸判定・可視判定を使うが、描画先の形状は軽量な検査用バッファ。GPU実行や最初のバッファ確保は測っていない。静止中の転送要求は99.7%減、移動時の処理量は同等。静止・移動・ズームを含む180視点で区画、地形・海の配置行列、海岸属性、川岸の表示が一致した。
...
Chrome（ヘッドレス、実GPUはANGLE / NVIDIA RTX 3070 Laptop）、1280×800、1画面、8秒の準備後に181フレーム＝180間隔を観測。森林の同じ位置で、間隔中央値は前後とも16.7ms、p95は17.5→17.3ms。JavaScript描画処理の平均は3.99→3.89ms。単回の観測であり、FPS向上の証拠とはしない。この場面では徒歩カメラの遮蔽修正も入り、変更後は葉に覆われていた人物と周囲が見える。元の画質・GLB・LOD距離・描画解像度を維持する。
