...
async function analyse(file) {
  ...
  for (let y = y0; y < y1; y++)
    for (let x = x0; x < x1; x++) {
      ...
      const bucket =
        ratio > 1.45 && g > b * 1.3 ? 'blade' : ratio > 0.75 && ratio <= 1.45 ? 'ground' : null;
      if (!bucket) continue;
      sum[bucket][0] += r;
      sum[bucket][1] += g;
      sum[bucket][2] += b;
  ...
  return page.evaluate(() => {
    ...
    for (const landscape of r.landscapes) {
      ...
      source.traverse((node) => {
        if (node.isMesh) original.push(node);
      });
      sources.push({
        key: landscape.key,
        sharedGeometry: landscape.levels[0].every(
          ({ mesh }, i) => mesh.geometry === original[i].geometry,
        ),
        sharedTexture: landscape.levels[0].every(
          ({ mesh }, i) => mesh.material.map === original[i].material.map,
        ),
        sourceUntinted: original.every(
          (node) => !node.material.customProgramCacheKey().includes('meadow-grass-palette'),
        ),
        allLodsTinted: landscape.levels
          .flat()
          .every(
            ({ mesh }) =>
              mesh.instanceColor &&
              mesh.material.customProgramCacheKey().includes('meadow-grass-palette'),
          ),
      });
      const mesh = landscape.levels[2][0].mesh;
      for (let i = 0; i < mesh.count; i++) {
        const matrix = mesh.instanceMatrix.array,
          colour = mesh.instanceColor.array;
...
try {
  ...
  // Review exact candidate GLBs before adopting them; all replacements stay in this QA browser.
  if (process.argv.includes('--root-candidates')) {
    const manifests = new Map();
    for (const [key, revision] of [
      ['meadow-grass', '05'],
      ['meadow-sprig', '04'],
    ]) {
      const folder = `output/model-generation/models/${key}/work/low-poly/candidate-${revision}`;
      const report = JSON.parse(await readFile(`${folder}/cut-report.json`, 'utf8'));
      const manifest = JSON.parse(await readFile(`public/models/${key}/asset.json`, 'utf8'));
      Object.assign(manifest, {
        sha256: report.candidate.sha256,
        bytes: report.candidate.bytes,
        triangles: report.triangles.after,
      });
      manifests.set(key, manifest);
      await page.route(`**/models/${key}/model.glb`, (route) =>
        route.fulfill({ path: `${folder}/candidate.glb`, contentType: 'model/gltf-binary' }),
      );
      await page.route(`**/models/${key}/asset.json`, (route) => route.fulfill({ json: manifest }));
    }
    const catalog = JSON.parse(await readFile('public/models/world-assets.json', 'utf8'));
    catalog.assets = catalog.assets.map((asset) => manifests.get(asset.modelKey) ?? asset);
    await page.route('**/models/world-assets.json', (route) => route.fulfill({ json: catalog }));
    facts.rootCandidates = [...manifests.values()].map(({ modelKey, sha256, triangles }) => ({
      modelKey,
      sha256,
      triangles,
    }));
  ...
  for (const source of paletteBefore.sources) {
    assert.ok(
      source.sharedGeometry &&
        source.sharedTexture &&
        source.sourceUntinted &&
        source.allLodsTinted,
      source.key,
    );
  }
  assert.ok(paletteBefore.sources.length >= 2);
  assert.ok(
    new Set(Object.values(paletteBefore.colours).map((c) => c.map((v) => v.toFixed(2)).join(',')))
      .size > 30,
  );
  ...
  const distances = await page.evaluate(() =>
    window.monsterReview.landscapes.map((landscape) => {
      const saved = [...landscape.distances];
      if (!landscape.surface && ['meadow-grass', 'meadow-sprig'].includes(landscape.key))
        landscape.distances[0] = 5;
      return saved;
    }),
  );
  await view(page, Math.PI * 0.5 + 0.01, 0.3, 2.2);
  facts.grassPalette.nearModels = await page.evaluate(() =>
    window.monsterReview.landscapes
  ...
  await page.evaluate(
    (saved) => window.monsterReview.landscapes.forEach((l, i) => (l.distances = saved[i])),
    distances,
  );
