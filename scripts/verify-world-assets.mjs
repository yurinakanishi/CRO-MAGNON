...
function canonicalizeZero(value) {
  ...
  if (value && typeof value === 'object') return Object.fromEntries(Object.entries(value).map(([key, entry]) => [key, canonicalizeZero(entry)]));
  return value;
}
...
for (const spec of catalog.assets) {
  ...
  for (const record of [manifest, ...manifest.lods]) {
    ...
    const inspection = JSON.parse(execFileSync(process.execPath, [resolve(root, 'scripts/inspect-glb.mjs'), path, ...(spec.kind === 'humanoid' ? ['--humanoid'] : []), ...(spec.kind === 'enemy' ? ['--enemy'] : []), ...(['humanoid', 'quadruped'].includes(spec.kind) ? ['--hunting'] : [])], { encoding: 'utf8' }));
    ...
    if (['enemy','humanoid','quadruped'].includes(spec.kind)) {
      assert.deepEqual(manifest.clips.map(clip => clip.name).sort(), inspection.animations.map(clip => clip.name).sort(), `${spec.key}: manifest clip names differ from GLB`);
      for (const clip of manifest.clips) {
        const actual = inspection.animations.find(animation => animation.name === clip.name);
        assert.ok(Math.abs(clip.seconds - actual.duration) < .0001, `${spec.key}/${clip.name}: manifest duration differs from GLB`);
        assert.equal(clip.loop, clip.name.endsWith('_Loop'), `${spec.key}/${clip.name}: invalid loop flag`);
      }
    }
  }
  if (['humanoid', 'quadruped', 'enemy'].includes(spec.kind)) {
    ...
    assert.equal(manifest.clips.length, spec.kind === 'humanoid' ? 9 : spec.kind === 'enemy' ? 6 : 5);
  }
...
const huntingKeys = ['cro-magnon-hunter','cro-magnon-woman','neanderthal-hunter','neanderthal-woman','woolly-mammoth'];
function glbParts(bytes) {
  const jsonLength = bytes.readUInt32LE(12), binStart = 20 + jsonLength;
  return { doc: JSON.parse(bytes.subarray(20, binStart).toString('utf8').trim()), binary: bytes.subarray(binStart + 8) };
}
for (const key of huntingKeys) {
  const oldBytes = await readFile(resolve(root, `output/hunting-animation/previous-delivery/${key}/model.glb`));
  const currentBytes = await readFile(resolve(root, `public/models/${key}/model.glb`));
  const old = glbParts(oldBytes), current = glbParts(currentBytes);
  ...
  assert.deepEqual(current.binary.subarray(0, old.binary.length), old.binary, `${key}: original mesh/UV/skin/texture/keyframe bytes must remain intact`);
  for (const field of ['nodes', 'meshes', 'skins', 'materials', 'textures', 'images']) assert.deepEqual(current.doc[field], old.doc[field], `${key}: ${field} changed`);
  assert.deepEqual(current.doc.animations.slice(0, -1), old.doc.animations, `${key}: existing clips changed`);
  if (key === 'cro-magnon-hunter') {
    assert.equal(hash(oldBytes), originalCroHash);
    const inspection = JSON.parse(execFileSync(process.execPath, [resolve(root, 'scripts/inspect-glb.mjs'), resolve(root, `public/models/${key}/model.glb`), '--humanoid', '--hunting'], { encoding: 'utf8' }));
    assert.equal(inspection.validation, 'passed'); assert.equal(inspection.animations.length, 9);
  }
}
console.log(JSON.stringify({ status: 'passed', models: results.length + 1, existingCroMagnonPreservedInArchive: true, huntingSourcesAndOldClipsPreserved: huntingKeys, assets: results }, null, 2));
