...
for(let i=0;i<3;i++){
  const socket=new WebSocket(`ws://127.0.0.1:3004/ws?room=WORLD-QA&name=BiomeObserver${i}&species=${i===1?'nea':'cro'}&gender=${i===2?'male':'female'}`);peers.push(socket);
...
console.log('Three passive biome observers active; q saves and exits');
...
async function finish(){
  ...
  report.status=report.maxPlayers===5&&report.sixthRefused&&Object.keys(report.biomes).length===5&&!report.staticPenetrations&&!report.dynamicPenetrations&&!report.mountMismatches&&!report.errors.length?'passed':'failed';
  await writeFile('assets/biome-network-qa.json',JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify(report));process.exit(report.status==='passed'?0:1);
}
