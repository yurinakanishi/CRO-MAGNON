...
// node scripts/desert-mage-canonical-sync.mjs plan
// node scripts/desert-mage-canonical-sync.mjs copy --report assets/desert-mage-canonical-sync.json
// node scripts/desert-mage-canonical-sync.mjs verify
...
export function createDesertMageSync(workspace = DEFAULT_WORKSPACE) { return createModelSync(KEY, workspace); }

export function createModelSync(modelKey, workspace = DEFAULT_WORKSPACE) {
  if (modelKey !== 'desert-fennec-mage') throw new Error('Unsupported canonical model key');
  const KEY=modelKey;
  workspace = path.resolve(workspace);
  if (path.basename(workspace) !== 'CRO-MAGNON') throw new Error('Workspace must be CRO-MAGNON');
  const source = path.join(workspace, 'output', 'model-generation', 'models', KEY);
  const canonical = path.join(path.dirname(workspace), 'threed-model-creation', 'models');
  const destination = path.join(canonical, KEY);
...
async function main() {
  ...
  if (!['plan', 'copy', 'verify'].includes(mode)) throw new Error('Usage: desert-mage-canonical-sync.mjs plan|copy|verify [--plan assets/desert-mage-canonical-sync-plan.json] [--report assets/desert-mage-canonical-sync.json]');
  const options = {};
  ...
  function reportPath(value) {
    ...
    if (!samePath(path.dirname(result), path.join(DEFAULT_WORKSPACE, 'assets')) || !/^desert-mage-canonical-sync[^/\\]*\.json$/.test(path.basename(result))) throw new Error('Plans and reports must be assets/desert-mage-canonical-sync*.json');
    return result;
  }
  ...
  const sync = createDesertMageSync(), planFile = reportPath(options['--plan'] || 'assets/desert-mage-canonical-sync-plan.json');
  let result;
