...
const folder = `output/playwright/${key}/asset-${revision}${dressed ? '-dressed' : ''}`;
...
const fire=new T.PointLight('#ffad57',35,16,1.6);fire.position.set(3,2.3,0);fire.visible=false;s.add(fire);
if(${dressed && key === 'camp-cave'}) {const p=await new T.TextureLoader().loadAsync('/qa-pigment.png');p.colorSpace=T.SRGBColorSpace;p.anisotropy=8;const a=await(await fetch('/models/camp-cave/asset.json')).json();const stone=await new T.TextureLoader().loadAsync(a.rockSurface.url);stone.colorSpace=T.SRGBColorSpace;stone.wrapS=stone.wrapT=T.RepeatWrapping;stone.anisotropy=8;(await import('/src/cave-materials.js')).prepareCaveMaterials(model,p,stone);}
if(${dressed && key === 'camp-mountain'}) (await import('/src/mountain-materials.js')).prepareMountainMaterials(model);
window.lightFire=lit=>fire.visible=lit;
...
window.review=(direction,clay=false,inside=false)=>{model.traverse(n=>{if(n.isMesh)n.material=clay?new T.MeshStandardMaterial({color:'#a2a2a2',roughness:.85,side:T.FrontSide}):originals.get(n);});const target=inside?new T.Vector3(0,2,0):centre;c.position.copy(inside?new T.Vector3(...direction):centre.clone().add(new T.Vector3(...direction).normalize().multiplyScalar(Math.max(size.x,size.y,size.z)*1.6)));c.lookAt(target);r.render(s,c);return{bounds:{min:box.min.toArray(),max:box.max.toArray()},triangles:r.info.render.triangles};};window.ready=true;
</script>`,
    }),
  );
