...
assert.ok(['snow-ground','ice-ground','desert-ground','volcanic-ground','glacier-spires','volcanic-cone'].includes(key));assert.match(revision,/^\d{2}$/);
...
if(old)assert.equal(old.sha256,hash);else catalog.assets.push({key,name:spec.name,kind:spec.kind,height:size.y,geometryResolution:1024,referenceVersion:1,status:'adopted-awaiting-game-QA',sha256:hash,delivery:manifest.url,subject:spec.subject??spec.prompt});
...
const requestPath=resolve(root,'assets/open-world-request.json'),request=await json(requestPath);Object.assign(request.models.find(a=>a.key===key),{generation:'completed',adoption:'exact-glb-reviewed-and-adopted',gameQa:'pending',revision,sha256:hash});await save(requestPath,request);
