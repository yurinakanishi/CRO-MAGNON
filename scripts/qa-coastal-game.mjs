...
const folder = process.argv[2] || 'output/playwright/coastal';
...
async function addPage(name) {
  ...
  await p.goto(`${base}/?room=COAST-QA&autostart=1`, { waitUntil: 'domcontentloaded' });
  await p
    .locator('#world[data-world-asset="ready"][data-character-asset="ready"]')
    .waitFor({ timeout: 90000 });
  return p;
}
const page = await addPage('貝場A');
...
const room = () => core.rooms.get('COAST-QA');
const actor = (name = '貝場A') => [...room().players.values()].find((p) => p.name === name);
...
try {
  for await (const line of rl) {
    try {
      ...
      if (q.type === 'compact')
        r = {
          players: core.snapshot(room()).players.map((p) => ({
            name: p.name,
            x: p.x,
            z: p.z,
            moving: p.moving,
            boatId: p.boatId,
            fishing: p.fishing,
            coastalActivity: p.coastalActivity,
          ...
          shellBeds: room().gulf.shellBeds,
          middens: room().gulf.middens,
        ...
        fps.push(
          await p
            ...
            .evaluate((c) => ({
              ...
              shells: c.dataset.shellInstances,
              middens: c.dataset.middenModels,
              tables: c.dataset.knappingTables,
              weapon: c.dataset.weapon,
            })),
        );
      ...
      if (q.type === 'addPeers') {
        for (let i = 0; i < q.count; i++) {
          const s = new WebSocket(
            `${base.replace('http', 'ws')}/ws?room=COAST-QA&name=Peer${i}&species=cro&resume=1`,
          );
          peers.push(s);
