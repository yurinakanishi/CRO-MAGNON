...
const key='desert-fennec-mage', model=resolve(root,'output/model-generation/models',key);
...
const runtimePaths=['shared/characters.mts','src/character-selection.ts','src/style.css','tests/characters.test.mjs','scripts/build-cloudflare.mjs'];
const runtimeEdits=[];
for(const path of runtimePaths){
  const original=await readFile(resolve(root,path),'utf8');
  assert.ok(original.includes('floppy-ear-mage')||original.includes('垂れ耳の魔法使い'),`Missing previous character in ${path}`);
  let content=original.replaceAll('floppy-ear-mage','desert-fennec-mage').replaceAll('垂れ耳の魔法使い','砂耳の魔法使い');
  if(path==='shared/characters.mts')content=content.replace('radius: 0.28','radius: 0.32');
  if(path==='src/style.css')content=content.replace("#615942 url('/models/desert-fennec-mage/portrait.png') 50% 10%", "#95714d url('/models/desert-fennec-mage/portrait.png') 50% 28%");
  runtimeEdits.push([path,content]);
}
...
const manifest={modelKey:key,name:'砂耳の魔法使い',kind:'humanoid',candidate:1,status:'reviewed-prototype',
  ...
  provenance:{provider:'Codex',claudeUsed:false,referenceGenerator:'Built-in imagegen original concept; image candidate 2 selected by user',
    ...
    candidateFile:portable(candidate),visualReview:portable(resolve(model,'qa/adoption-review.json')),replaces:'floppy-ear-mage'}};
...
const slot=catalog.assets.findIndex(a=>a.key==='floppy-ear-mage');
...
catalog.assets[slot]={key,name:manifest.name,kind:'humanoid',height:size.y,geometryResolution:1024,referenceVersion:1,
  status:'adopted-awaiting-game-QA',sha256:hash,delivery:manifest.url,subject:'Small sandy-furred fennec mage with upright triangular ears, green eyes, fox muzzle, primitive hide/fur clothes, bare hands and feet, and a tail.',replaces:'floppy-ear-mage'};
...
for(const [path,content] of runtimeEdits)await writeFile(resolve(root,path),content);
const modelState=await json(resolve(model,'model.json'));
Object.assign(modelState,{lifecycle_status:'adopted-awaiting-game-QA',sha256:hash,revision:process.revision});
await save(resolve(model,'model.json'),modelState);
const requestPath=resolve(root,'assets/desert-fennec-mage-request.json'),request=await json(requestPath);
