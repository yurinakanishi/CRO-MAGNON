...
const persist=()=>writeFile('assets/female-network-qa.json',JSON.stringify({...report,animalClips:[...clips]},null,2)+'\n');
async function stop(){timers.forEach(clearInterval);sockets.forEach(s=>s.close());await persist();process.exit(0);}
process.on('SIGINT',stop);process.on('SIGTERM',stop);
for(let i=0;i<4;i++) {
  const socket=new WebSocket(`${url}?room=${room}&name=Avatar-QA-${i+1}&species=${i%2?'nea':'cro'}&gender=${i<2?'female':'male'}`);sockets.push(socket);
  ...
  socket.on('message',async bytes=>{
    ...
    report.characters=[...new Set([...report.characters,...m.players.map(p=>`${p.species}/${p.gender}`)])];report.states++;report.maxPlayers=Math.max(report.maxPlayers,m.players.length);report.species=[...new Set([...report.species,...m.players.map(p=>p.species)])];
...
console.log(`Owned QA clients PID ${process.pid}; auto-close after 6 minutes.`);
timers.push(setTimeout(stop,360000));
