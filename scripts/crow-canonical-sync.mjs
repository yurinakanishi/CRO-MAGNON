...
// node scripts/crow-canonical-sync.mjs plan
// node scripts/crow-canonical-sync.mjs copy --report assets/crow-canonical-sync.json
// node scripts/crow-canonical-sync.mjs verify
...
export function createModelSync(modelKey, workspace = DEFAULT_WORKSPACE) {
  if (!['crow-shaman','cat-kunoichi','bear-mage','kunoichi-katana','snow-ground','ice-ground','desert-ground','volcanic-ground','glacier-spires','volcanic-cone'].includes(modelKey)) throw new Error('Unsupported canonical model key');
  const KEY=modelKey;
...
async function main() {
  ...
  if (!['plan', 'copy', 'verify'].includes(mode)) throw new Error('Usage: crow-canonical-sync.mjs plan|copy|verify [--plan assets/crow-canonical-sync-plan.json] [--report assets/crow-canonical-sync.json]');
  const options = {};
  ...
  function reportPath(value) {
    ...
    if (!samePath(path.dirname(result), path.join(DEFAULT_WORKSPACE, 'assets')) || !/^crow-canonical-sync[^/\\]*\.json$/.test(path.basename(result))) throw new Error('Plans and reports must be assets/crow-canonical-sync*.json');
    return result;
  }
  ...
  const sync = createCrowSync(), planFile = reportPath(options['--plan'] || 'assets/crow-canonical-sync-plan.json');
  let result;
